...
LOG_FILE="${SCRIPT_DIR}/logs/training_async_forward_kl_active.log"
exec > >(tee -a "${LOG_FILE}") 2>&1
echo "===== $(date '+%Y-%m-%d %H:%M:%S') train_student_async_forward_kl start ====="
...
STUDENT_SAVE_PATH="${STUDENT_SAVE:-${OUTPUT_ROOT}/student_async_forward_kl}"
...
[[ "${OPD_TOP_LOGPROBS_NUM}" =~ ^[0-9]+$ ]] || {
  echo "OPD_TOP_LOGPROBS_NUM must be a positive integer, got '${OPD_TOP_LOGPROBS_NUM}'" >&2
  exit 1
}
(( OPD_TOP_LOGPROBS_NUM > 0 )) || {
  echo "OPD_TOP_LOGPROBS_NUM must be > 0, got '${OPD_TOP_LOGPROBS_NUM}'" >&2
  exit 1
}
...
try:
    ...
    raise SystemExit(1)
raise SystemExit(0 if 0.0 <= beta <= 1.0 else 1)
PY
then
  echo "OPD_JSD_BETA must be a float in [0,1], got '${OPD_JSD_BETA}'" >&2
  exit 1
fi
...
ray job submit --address="${RAY_JOB_ADDRESS}" \
  ...
  --wandb-group "${WANDB_GROUP:-opd-397-32b-student-async-forward-kl}" \
